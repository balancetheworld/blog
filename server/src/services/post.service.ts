...
import { db, posts, categories, tags, postTags, comments, likes, type Post, type NewPost } from '@/db'
...
class PostService {
  ...
  async create(input: CreatePostInput): Promise<Post> {
    ...
    // 如果没有提供 slug，从标题生成
    let slug = input.slug || this.slugify(input.title)

    // 如果 slug 为空（例如纯中文标题），使用 ID
    if (!slug) {
      slug = postId
    }
